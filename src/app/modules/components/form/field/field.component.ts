...
import { FieldValidationErrorDisplay, ICustomFieldValidationError, IFieldKeyEvent } from "./field";
...
export class FieldComponent implements OnInit, AfterViewInit, OnDestroy {
  constructor(
    ...
    @Optional() @Inject(FIELD_I18N_SERVICE) private i18nService?: IFieldI18nService,
  ) {
  ...
  private ngAfterViewInit$ = new BehaviorSubject<HTMLInputElement[]>(undefined);
  ...
  private get valueControl() {
    return this.valueControlName ? this.formGroup?.get(this.valueControlName) as FormControl : undefined;
  }
  ...
  ngOnInit() {
    ...
    this.fieldService.ngAfterViewInit$ = this.ngAfterViewInit$.pipe(filter(inputs => inputs !== undefined));
    ...
    control
      ...
      .subscribe(({ required, minlength, maxlength, min, max, email, ...custom }) => {
        if (required)
          this.requiredErrorMessage = this.i18nService.getRequiredErrorMessage(this.label);
        if (minlength)
          this.minLengthErrorMessage = this.i18nService.getMinLengthErrorMessage(
            this.label, 
            minlength
          );
        if (maxlength)
          this.maxLengthErrorMessage = this.i18nService.getMaxLengthErrorMessage(
            this.label, 
            maxlength
          );
        if (min)
          this.minNumberErrorMessage = this.i18nService.getMinNumberErrorMessage(
            this.label, 
            min
          );
        if (max)
          this.maxNumberErrorMessage = this.i18nService.getMaxNumberErrorMessage(
            this.label, 
            max
          );
        if (email)
          this.emailErrorMessage = this.i18nService.getEmailErrorMessage(this.label);

        this.customValidationErrors = Object.entries(custom)
          .map(([error, props]) => ({
            error,
            message: this.i18nService.getCustomErrorMessage(error, this.label, props),
          }))
          .filter(({ error, message }) => {
            if (message === null || message === undefined) {
              console.warn('No custom field validation error message found for error', error);
              return false;
            }
            return true;
          });
