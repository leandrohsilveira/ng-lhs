...
import { DefaultFieldSuggestionI18nService, FIELD_SUGGESTION_I18N_SERVICE, IFieldSuggestionI18nService } from './field-suggestion-i18n.service';
...
export class FieldSuggestionComponent implements OnInit, OnDestroy, ControlValueAccessor {

  constructor(
    private fieldService: FieldService,
    @Optional() @Inject(FIELD_SUGGESTION_I18N_SERVICE) 
  ...
  set selectedSuggestion(selectedSuggestion) {
    if (this._selectedSuggestion !== selectedSuggestion) {
      ...
      if (selectedSuggestion)
        this.fieldService?.control?.setValue(this.getSuggestionLabel(selectedSuggestion));
    }
  }
  ...
  ngOnInit() {
    if (!this.minSearchLengthText) {
      this.minSearchLengthText = this.i18nService.getMinSearchLengthMessage(this.minSearchLength);
    }
    if (!this.noSuggestionsFoundText) {
      this.noSuggestionsFoundText = this.i18nService.getNoSuggestionFoundMessage();
    }
    ...
    this.fieldService
      ?.control
      ?.valueChanges.pipe(
        filter(value => value !== this.getSuggestionLabel(this.selectedSuggestion)),
        tap(() => {
          this.open = true;
  ...
  registerOnChange(next: any) {
    this.selectedSuggestionChange.pipe(takeUntil(this.ngOnDestroy$)).subscribe({ next });
  }
  ...
  onClick(index: number) {
    if (this.open) {
      ...
      this.open = false;
      this.selectedSuggestion = this.suggestions[this.cursorSuggestionIndex].value;
    }
  }
  ...
  private handleEnter(e: Event) {
    if (this.open && this.suggestionsLength > 0) {
      e.preventDefault();
      this.open = false;
      this.selectedSuggestion = this.suggestions[this.cursorSuggestionIndex]?.value;
    }
