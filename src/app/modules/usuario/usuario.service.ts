...
import { IUsuarioEditar, IUsuarioIncluir, IUsuarioListar, IUsuarioVisualizar, USUARIOS } from "./usuario";
  ...
  recuperarPorId(id: string): Observable<IUsuarioVisualizar> {
    return simulateAsync().pipe(
      ...
      switchMap(({ papeis: chaves, ...dados }) =>
        this.papelService.buscarPorChaves(chaves).pipe(
          map(papeis => ({ ...dados, papeis }))
        )
      )
    )
  }
