...
const UsuarioFormValidators = {
      ...
      if (!editar) {
        ...
        if (control.parent instanceof FormGroup) {
          const confirmarSenha =
            control.parent.controls[confirmarSenhaName]?.value
  ...
  arrayMinLength(minLength: number) {
    return (control: AbstractControl) =>
      control instanceof FormArray && control.length >= minLength
        ? null
        : {arrayMinLength: true}
  },
  ...
  termosDeUso(editar: boolean) {
    return (control: AbstractControl) =>
      editar ? null : Validators.requiredTrue(control)
  },
}
...
export class UsuarioFormGroup extends FormGroup {
  constructor(dados?: IUsuarioFormValue, editar = false) {
    super({
      ...
      email: new FormControl(dados?.email, [
        Validators.required,
        Validators.email,
      ]),
      username: new FormControl({value: dados?.username, disabled: editar}, [
        Validators.required,
        Validators.maxLength(12),
      ]),
      senha: new FormControl(null, [
        UsuarioFormValidators.senha(editar, 'confirmarSenha'),
      ]),
      confirmarSenha: new FormControl(null, [
        UsuarioFormValidators.senha(editar, 'senha'),
      ]),
      termosDeUsoAceito: new FormControl(false, [
        UsuarioFormValidators.termosDeUso(editar),
      ]),
